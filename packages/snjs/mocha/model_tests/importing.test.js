...
describe('importing', function () {
  ...
  it('importing same note many times should create only one duplicate', async function () {
    ...
    expectedItemCount++
    expect(application.navigation.getNotes().length).to.equal(2)
    const imported = application.navigation.getNotes().find((n) => n.uuid !== notePayload.uuid)
  ...
  it('importing a tag with lesser references should not create duplicate', async function () {
    ...
    expect(application.itemManager.getTags().length).to.equal(1)
  ...
  it('importing data with differing content should create duplicates', async function () {
    ...
    await application.itemManager.emitItemsFromPayloads(pair, PayloadEmitSource.LocalChanged)
    expectedItemCount += 2
    const note = application.navigation.getNotes()[0]
    const tag = application.itemManager.getTags()[0]
    const mutatedNote = new DecryptedPayload({
      ...notePayload,
      content: {
        ...notePayload.content,
    ...
    const newNote = application.navigation.getNotes().find((n) => n.uuid !== notePayload.uuid)
    const newTag = application.itemManager.getTags().find((t) => t.uuid !== tagPayload.uuid)
  ...
  it('should duplicate notes by alternating UUIDs when dealing with conflicts during importing', async function () {
    ...
    expect(application.navigation.getNotes().length).to.equal(1)
    expect(application.navigation.getNotes()[0].uuid).to.not.equal(note.uuid)
  })
  ...
  it('should not import data from 003 encrypted payload if an invalid password is provided', async function () {
    ...
    expect(application.navigation.getNotes().length).to.equal(0)
  })
  ...
  it('importing another accounts notes/tags should correctly keep relationships', async function () {
    ...
    const importedNote = application.navigation.getNotes()[0]
    const importedTag = application.itemManager.getTags()[0]
