...
describe('Test 003 encryption', () => {
  ...
  let _keyParams: SNRootKeyParams, _key: SNRootKey;

  let sharedApplication: SNApplication;
  const protocol003 = new SNProtocolOperator003(new SNCrypto());

  // runs once before all tests in this block
  beforeAll(async () => {
    sharedApplication = await Factory.createAppWithRandNamespace(Environment.Web, Platform.LinuxWeb);
  ...
  afterAll(() => {
    sharedApplication.deinit(DeinitSource.SignOut);
  });
  ...
  it('cost minimum should throw', () => {
    expect(() => {
      sharedApplication.protocolService.costMinimumForVersion(ProtocolVersion.V003);
  ...
  it('generates valid keys for registration', async () => {
    ...
    expect(key.keyParams.content.pw_cost).toBeUndefined;
    expect(key.keyParams.content.pw_salt).toBeUndefined;
    expect(key.keyParams.content.identifier).toBeDefined;
  });
  ...
  it('can decrypt item generated with web version 3.3.6', async () => {
    ...
    const keyParams = sharedApplication.protocolService.createKeyParams({
      pw_nonce:
        '31107837b44d86179140b7c602a55d694243e2e9ced0c4c914ac21ad90215055',
      identifier: identifier,
      version: ProtocolVersion.V003,
    });
    const key = await protocol003.computeRootKey(password, keyParams);
    ...
    expect(decrypted.content!.title).toEqual('Secret key');
    expect(decrypted.content!.text).toEqual('TaW8uq4cZRCNf3e4L8c7xFhsJkJdt6');
  });
