...
export class SNComponentManager extends PureService {
  ...
  configureForGeneralUsage() {
    this.modelManager.addMappingObserver("*", async (allItems, validItems, deletedItems, source, sourceKey) => {
      ...
      */
      if(syncedComponents.length > 0 && source != PayloadSoures.RemoteSaved) {
  ...
  jsonForItem(item, component, source) {
    ...
    // Changes are always metadata updates if the mapping source is PayloadSoures.RemoteSaved || source == PayloadSoures.LocalSaved.
    //
    if(source && (source == PayloadSoures.RemoteSaved || source == PayloadSoures.LocalSaved)) {
    ...
    this.runWithPermissions(component, requiredPermissions, async () => {
      ...
      */
      ...
      let localItems = await this.modelManager.mapPayloadsToLocalItems({
        paylods: payloads,
        source: PayloadSoures.ComponentRetrieved,
      ...
      for(let responseItem of responseItems) {
        ...
        if(!item.locked) {
          ...
          await this.modelManager.setItemDirty(item, true, true, PayloadSoures.ComponentRetrieved, component.uuid);
        }
      }
    ...
    this.runWithPermissions(component, requiredPermissions, async () => {
      ...
      if(didConfirm) {
        ...
        for(const itemData of itemsData) {
          ...
          this.modelManager.notifyMappingObservers([model], PayloadSoures.RemoteSaved);
        }
