...
import { EncryptionIntentFile } from '@Protocol/intents';
...
export class SFModelManager {
  ...
  async alternateUUIDForItem(item) {
    ...
    newItem.uuid = await protocolManager.crypto.generateUUID();
  ...
  async createDuplicateItemFromResponseItem(itemResponse) {
    ...
    itemResponseCopy.uuid = await protocolManager.crypto.generateUUID();
  ...
  */
  ...
  async getAllItemsJSONData(keys, keyParams, returnNullIfEmpty) {
    return this.getJSONDataForItems(this.allItems, keys, keyParams, returnNullIfEmpty);
  }

  async getJSONDataForItems(items, keys, keyParams, returnNullIfEmpty) {
    return Promise.all(items.map((item) => {
      return protocolManager.generateExportParameters({item, keys, keyParams, intent: EncryptionIntentFile})
    })).then((items) => {
