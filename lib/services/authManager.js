...
export class SFAuthManager {
  ...
  async register({url, email, password, ephemeral}) {
    ...
    const results = await this.protocolManager.createRootKey({identifier: email, password});
    const rootKey = results.key;
    ...
    return this.httpManager.postAbsolute(requestUrl, params).then(async (response) => {
      ...
      await this.keyManager.createNewItemKey();
  ...
  async changePassword({url, email, current_server_pw, newKey, newKeyParams}) {
    if(this.isLocked()) {
    ...
    const newServerPw = newKey.serverAuthenticationValue;
