...
export class SFAuthManager {
  ...
  async getKeyParams() {
    if(!this._keyParams) {
      var data = await this.storageManager.getItem("keyParams");
      this._keyParams = JSON.parse(data);
    }

    if(this._keyParams && !this._keyParams.version) {
      this._keyParams.version = await this.defaultProtocolVersion();
    }
  ...
  async defaultProtocolVersion() {
    ...
    if(keys && keys.ak) {
      // If there's no version stored, and there's an ak, it has to be 002. Newer versions would have thier version stored in keyParams.
      return "002";
    } else {
      return "001";
    }
  ...
  async protocolVersion() {
    var keyParams = await this.getKeyParams();
  ...
  register(url, email, password) {
    return new Promise(async (resolve, reject) => {
      ...
      var params = merge({password: keys.serverAuthenticationValue, email: email}, keyParams);
      params['api'] = SFHttpManager.getApiVersion();
  ...
  async handleAuthResponse(response, email, url, keyParams, keys) {
    if(url) { await this.storageManager.setItem("server", url);}
    this._keyParams = keyParams;
    await this.storageManager.setItem("keyParams", JSON.stringify(keyParams));
