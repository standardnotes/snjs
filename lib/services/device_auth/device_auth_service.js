...
export class DeviceAuthService extends PureService {
  ...
  /**
   * @access public
   * @returns {object} {valid, validationArtifacts}
   */
  async validateChallengeResponse(response) {
    if (response.challenge === Challenges.LocalPasscode) {
      ...
      return { valid: response.value === true };
    }
  ...
  async handleChallengeResponse(response) {
    if (response.challenge === Challenges.LocalPasscode) {
      let wrappingKey = response.validationArtifacts && response.validationArtifacts.wrappingKey;
      if (!wrappingKey) {
