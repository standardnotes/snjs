...
import { ENCRYPTION_INTENT_LOCAL_STORAGE_PREFER_ENCRYPTED, ENCRYPTION_INTENT_SYNC } from '@Protocol/intents';
import {
...
export class SNSyncManager {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if(nonDeletedItems.length > 0) {
        let params = await Promise.all(nonDeletedItems.map(async (item) => {
          ...
          const itemParams = await this.protocolManager.payloadByEncryptingPayload({
            payload: payload,
  ...
  async handleSyncSuccess(syncedItems, response, options) {
    ...
    if(potentialRetrievedConflicts.length > 0) {
      await this.protocolManager.payloadsByDecryptingPayloads(potentialRetrievedConflicts);
  ...
  async handleItemsResponse(responseItems, omitFields, source, keyRequest) {
    ...
    await this.protocolManager.payloadsByDecryptingPayloads(payloads);
  ...
  */
  async handleConflictsResponse(conflicts) {
    ...
    for(let conflict of conflicts) {
      ...
      let serverItemResponse = conflict.server_item || conflict.unsaved_item;
      await this.protocolManager.payloadsByDecryptingPayloads([serverItemResponse]);
  ...
  async deprecated_handleUnsavedItemsResponse(unsaved) {
    ...
    for(let mapping of unsaved) {
      ...
      await this.protocolManager.payloadsByDecryptingPayloads([itemResponse]);
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        this.httpManager.postAuthenticatedAbsolute(await this.getSyncURL(), params, async (response) => {
          ...
          await this.protocolManager.payloadsByDecryptingPayloads(incomingItems);
