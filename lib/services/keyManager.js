...
export class SNKeyManager {
  ...
  */
  async unwrapRootKey({wrappingKey}) {
    const wrappedKey = await this.getWrappedKeyFromStorage();
    await this.protocolManager.payloadByDecryptingPayload({item: wrappedKey, key: wrappingKey});
  ...
  */
  async keyToUseForDecryptionOfPayload({payload}) {
    ...
    if(payload.items_key_id) {
      const itemsKey = this.allItemsKeys.find((key) => key.uuid === payload.items_key_id);
      return itemsKey;
    }
