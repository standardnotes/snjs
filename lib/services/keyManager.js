...
} from '@Lib/services/storageManager';

/** i.e No account and no passcode */
const KEY_MODE_ROOT_KEY_NONE         = 0;
/** i.e Account but no passcode */
const KEY_MODE_ROOT_KEY_ONLY         = 1;
/** i.e Account plus passcode */
const KEY_MODE_ROOT_KEY_PLUS_WRAPPER = 2;
/** i.e No account, but passcode */
const KEY_MODE_WRAPPER_ONLY          = 3;

export class SNKeyManager {
  constructor({modelManager, storageManager, protocolManager, syncManager}) {
  ...
  async initialize() {
    const wrappedKey = await this.getRootKeyWrapperKeyParams();
    const rootKey = await this.getRootKeyFromKeychain();
    const hasWrapper = !isNullOrUndefined(wrappedKey);
    const hasRootKey = !isNullOrUndefined(rootKey);
  ...
  */
  async unwrapRootKey({wrappingKey}) {
    ...
    if(this.keyMode === KEY_MODE_WRAPPER_ONLY) {
      /**
      * The only way to verify if this key is correct
      * is to attempt to decrypt storage with it
      */
      const valid = await this.storageManager.canDecryptWithKey(wrappingKey);
      if(valid) {
