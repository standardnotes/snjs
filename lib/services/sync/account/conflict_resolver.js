...
export class AccountSyncConflictResolver {
  ...
  findCurrentItemPayload(uuid) {
    return this.masterCollection.findPayload(uuid);
  }
  ...
  /**
   * In sync_conflict, we receive conflict.server_item, or conflict.item for legacy servers.
   */
  async handleConflictingData() {
    let rawServerValue = this.conflict.server_item  || this.conflict.item;
    ...
    if(rawServerValue.deleted || currentItemPayload.deleted) {
      ...
      const contentExcludingReferencesDiffers = !SFItem.AreItemContentsEqual({
        ...
        keysToIgnore: tempCurrentItem.contentKeysToIgnoreWhenCheckingEquality().concat(['references']),
        appDataKeysToIgnore: tempCurrentItem.appDatacontentKeysToIgnoreWhenCheckingEquality()
      })
  ...
  async handleConflictingUuid() {
    let rawServerValue = this.conflict.unsaved_item || this.conflict.item;
    const payload = CreatePayloadFromAnyObject({
      object: rawServerValue
    });
