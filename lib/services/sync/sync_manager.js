...
import { PayloadCollection, CreateSourcedPayloadFromObject } from '@Protocol/payloads';
...
export const SYNC_MODE_DOWNLOAD_FIRST  = 2;
...
export class SNSyncManager extends SNService {
  ...
  async sync({timingStrategy, mode, checkIntegrity} = {}) {
    ...
    } else if(useMode === SYNC_MODE_DOWNLOAD_FIRST) {
      uploadPayloads = [];
    }
  ...
  async handleSuccessServerResponse({operation, response}) {
    ...
    for(const payload of response.allProcessedPayloads) {
      if(!payload.fields().includes(ITEM_PAYLOAD_CONTENT)) {
        /** Some payload types do not contiain content (like remote saved) */
        continue;
      }
