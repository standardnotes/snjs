...
export class SNSyncManager extends SNService {
  ...
  async loadDatabasePayloads(rawPayloads) {
    ...
    const itemsKeysPayloads = payloads.filter((payload) => payload.content_type === 'SN|ItemsKey');
    subtractFromArray(payloads, itemsKeysPayloads);
    const decryptedItemsKeys = await this.protocolManager.payloadsByDecryptingPayloads({
  ...
  async popItemsNeedingSync() {
     ...
     * Anyone marking the item dirty after this will cause it to sync again.
     */
