...
export class SNComponentManager {
  ...
  configureForGeneralUsage() {
    this.modelManager.addItemSyncObserver("component-manager", "*", (allItems, validItems, deletedItems, source, sourceKey) => {
      ...
      */
      if(syncedComponents.length > 0 && source != SNModelManager.MappingSourceRemoteSaved) {
  ...
  jsonForItem(item, component, source) {
    ...
    // Changes are always metadata updates if the mapping source is SNModelManager.MappingSourceRemoteSaved || source == SNModelManager.MappingSourceLocalSaved.
    //
    if(source && (source == SNModelManager.MappingSourceRemoteSaved || source == SNModelManager.MappingSourceLocalSaved)) {
  ...
  async handleSaveItemsMessage(component, message) {
    ...
    this.runWithPermissions(component, requiredPermissions, async () => {
      ...
      */
      ...
      let localItems = await this.modelManager.mapResponseItemsToLocalModels(responseItems, SNModelManager.MappingSourceComponentRetrieved, component.uuid);
      ...
      for(let responseItem of responseItems) {
        ...
        if(!item.locked) {
          ...
          this.modelManager.setItemDirty(item, true, true, SNModelManager.MappingSourceComponentRetrieved, component.uuid);
        }
      }
    ...
    this.runWithPermissions(component, requiredPermissions, async () => {
      ...
      if(didConfirm) {
        ...
        for(var itemData of itemsData) {
          ...
          this.modelManager.notifySyncObserversOfModels([model], SNModelManager.MappingSourceRemoteSaved);
        }
