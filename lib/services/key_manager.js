...
export class SNKeyManager extends PureService {
  ...
  */
   ...
   * @returns {object}  { valid, validationArtificats : { wrappingKey : object } }
   */
  async validateAccountPassword(password) {
    ...
    if (valid) {
      return { valid, validationArtifacts: { rootKey: key } };
    } else {
      return { valid: false };
    }
  }
  ...
  async validatePasscode(passcode) {
    ...
    if (valid) {
      return { valid, validationArtifacts: { wrappingKey: key } };
    } else {
      return { valid: false };
    }
  }
