  ...
  async generateDecryptedParameters({encryptedParameters, key}) {
    ...
    if(!content) {
      return CopyEncryptionParameters({
        encryptionParameters: encryptedParameters,
        override: {
          [ITEM_PAYLOAD_ERROR_DECRYPTING]: true,
          [ITEM_PAYLOAD_ERROR_DECRYPTING_CHANGED]: !encryptedParameters.errorDecrypting
        }
      });
    } else {
      return CopyEncryptionParameters({
        encryptionParameters: encryptedParameters,
        override: {
          [ITEM_PAYLOAD_CONTENT]: JSON.parse(content),
          [ITEM_PAYLOAD_ERROR_DECRYPTING]: false,
          [ITEM_PAYLOAD_ERROR_DECRYPTING_CHANGED]: encryptedParameters.errorDecrypting === true
        }
      });
    }
  }
