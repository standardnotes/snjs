...
export class SNProtocolOperator002 extends SNProtocolOperator001 {
  ...
  async encryptItem({item, key, keyParams}) {
    ...
    params.enc_item_key = await this._private_encryptString(item_key, key.masterKey, key.dataAuthenticationKey, item.uuid, keyParams);
  ...
  async decryptItem({item, key}) {
    ...
    const keyParams = this.encryptionComponentsFromString(encryptedItemKey, key.masterKey, key.dataAuthenticationKey);
  ...
  async generateExportParameters({item, key, keyParams, includeDeleted, intent}) {
    if(key && !keyParams) { throw "keyParams must be supplied if supplying key." }
