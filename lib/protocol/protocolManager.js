...
export class SNProtocolManager {
  ...
  */
  ...
  async payloadsByDecryptingPayloads(encryptedPayloads, throws) {
    ...
    for(const encryptedPayload of encryptedPayloads) {
      ...
      try {
        const decryptedPayload = await this.payloadByDecryptingPayload({payload: encryptedPayload});
  ...
  async decryptItemsWaitingForKeys() {
    ...
    const decrypted = await this.payloadsByDecryptingPayloads({payloads: payloads});
