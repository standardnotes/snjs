...
export class SNProtocolManager {
  ...
  */
  ...
  // async generateEncryptedItemPayload({item, intent}) {
  //   if(
  ...
  //   const operator = this.operatorForVersion(version);
  //   return operator.generateEncryptedItemPayload({item, key});
  // }
  ...
  async decryptItemPayload({payload, key}) {
    ...
    const operator = this.operatorForVersion(version);
    return operator.decryptItemPayload({item, key});
  }

  async decryptMultipleItemPayloads(items, throws) {
    for(const item of items) {
      ...
      try {
        await this.decryptItemPayload({item});
      } catch (e) {
  ...
  async generateExportParameters({item, key, includeDeleted, intent}) {
    ...
    const operator = this.operatorForVersion(version);
    const parameters = await operator.generateExportParameters({item, key, includeDeleted, intent});
    if(!parameters) {
