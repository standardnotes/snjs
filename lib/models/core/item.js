...
export class SFItem {
  ...
  */
  ...
  /**
   * Returns the SNItemsKey used to encrypt this object
   */
  get encryptingKey() {
    const filtered = this.referencedObjects.filter((item) => item.content_type === SN_ITEMS_KEY_CONTENT_TYPE);
    if(filtered.length > 1)  {
      throw `Item ${this.uuid} has two or more encrypting keys, when it should have only one.`;
  ...
  updateLocalRelationships() {
    ...
    var uuids = references.map((ref) => {return ref.uuid});
    this.referencedObjects.slice().forEach((referencedItem) => {
      if(!uuids.includes(referencedItem.uuid)) {
        remove(this.referencedObjects, {uuid: referencedItem.uuid});
        referencedItem.setIsNoLongerBeingReferencedBy(this);
      }
    })
  }
