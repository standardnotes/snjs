  ...
  async __params() {
    ...
    if(!this.item.errorDecrypting) {
      ...
      if(this.keys && !doNotEncrypt) {
        var encryptedParams = await cryptoManager.encryptItem(this.item, this.keys, this.auth_params);
        ...
        params.content = this.forExportFile ? this.item.createContentJSONFromProperties() : "000" + await cryptoManager.crypto.base64(JSON.stringify(this.item.createContentJSONFromProperties()));
