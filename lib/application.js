...
import {
  ApplicationEvents, ApplicationStages, applicationEventForSyncEvent, Challenges
} from '@Lib';
...
export class SNApplication {
  ...
  */
   ...
   * @param {async|function} callbacks.requiresChallengeResponses
   * Return an array of ChallengeResponse for each Challenge.
   ...
   */
  async upgradeProtocolVersion(callbacks) {
    ...
    while (pendingChallenges.length > 0) {
      ...
      const responses = await callbacks.requiresChallengeResponses(pendingChallenges);
      for (const response of responses) {
        const valid = await this.deviceAuthService.validateChallengeResponse(response);
        if (valid) {
      ...
      if(failedResponses.length > 0) {
        await callbacks.handleFailedChallengeResponses(failedResponses);
      }
    }
    const errors = [];
    if (hasPasscode) {
      /* Upgrade passcode version */
      const passcodeResponse = validResponses.find((r) => r.challenge === Challenges.LocalPasscode);
      const passcode = passcodeResponse.value;
      await this.changePasscode(passcode);
    }
    if (hasAccount) {
      /* Upgrade account version */
      const accountResponse = validResponses.find((r) => r.challenge === Challenges.AccountPassword);
  ...
  async changePassword({ email, currentPassword, newPassword }) {
    ...
    const result = await this.sessionManager.changePassword({
      currentPassword, currentKeyParams, newPassword
    });
    if (!result.response.error) {
      await this.keyManager.setNewRootKey({
        key: result.rootKey,
