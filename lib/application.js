...
import { SERVICE_EVENT_STORAGE_DECRYPTED } from '@Services/events';
...
export class SNApplication {
   ...
   */
  async launch({callbacks, ut_awaitDatabaseLoad}) {
    ...
    await this.handleStage(
      stages.APPLICATION_STAGE_BEFORE_LOCAL_DATA_LOAD
    );
    /**
  ...
  async handleLaunchAuthentication(callbacks, previousResponses = []) {
    ...
    await this.handleStage(
      stages.APPLICATION_STAGE_AFTER_APP_UNLOCKED
    );
  }
  ...
  async createServices() {
    ...
    await this.handleStage(
      stages.APPLICATION_STAGE_BEFORE_SERVICES_INIT
    );
    ...
    this.storageManager.addEventObserver(async (eventName) => {
      if(eventName === SERVICE_EVENT_STORAGE_DECRYPTED) {
        await this.handleStage(
          stages.APPLICATION_STAGE_AFTER_STORAGE_DECRYPTED
        );
      }
    })
    await this.storageManager.initializeFromDisk();
    await this.handleStage(
      stages.APPLICATION_STAGE_AFTER_STORAGE_SERVICE_INIT
    );
    ...
    await this.handleStage(
      stages.APPLICATION_STAGE_AFTER_SERVICES_INIT
    );
  }
    ...
    this.syncManager.addEventObserver(async (event, data) => {
      if(event === SYNC_EVENT_FULL_SYNC_COMPLETED) {
        if(!this.keyManager.getDefaultItemsKey()) {
          await this.keyManager.createItemsKey();
