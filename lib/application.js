...
export class SNApplication {
  ...
  async prepareForLaunch({ callbacks }) {
    ...
    this.ready = true;
    this.notifyEvent(ApplicationEvents.ApplicationReady);
    this.launchCallbacks.onReady && await this.launchCallbacks.onReady();
  }
  ...
  onReady(callback) {
    if (this.ready) {
      ...
      const event = ApplicationEvents.ApplicationReady;
  ...
  getUser() {
    if(!this.ready) {
      throw 'Attempting to access user before application ready';
    }
