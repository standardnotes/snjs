...
__webpack_require__.d(__webpack_exports__, "HistorySession", function() { return /* reexport */ history_session_HistorySession; });
__webpack_require__.d(__webpack_exports__, "ItemHistory", function() { return /* reexport */ item_history_ItemHistory; });
...
const API_MESSAGE_GENERIC_CHANGE_PW_FAIL = "Something went wrong while changing your password.\n                                                      Your password was not changed. Please try again.";
...
const API_MESSAGE_GENERIC_TOKEN_REFRESH_FAIL = "A server error occurred while trying to refresh your session.\n                                                      Please try again.";
...
class component_manager_SNComponentManager extends pure_service["a" /* PureService */] {
  ...
  jsonForItem(item, component, source) {
    const isMetadatUpdate = source === sources["a" /* PayloadSource */].RemoteSaved || source === sources["a" /* PayloadSource */].LocalSaved;
  ...
  async handleSaveItemsMessage(component, message) {
    const responsePayloads = message.data.items;
    const requiredPermissions = [];
    ...
    this.runWithPermissions(component.uuid, requiredPermissions, async () => {
      this.removePrivatePropertiesFromResponseItems(responsePayloads, component, true);
      /* Filter locked items */
...
class singleton_manager_SNSingletonManager extends pure_service["a" /* PureService */] {
  ...
  addObservers() {
    this.removeItemObserver = this.itemManager.addObserver(content_types["a" /* ContentType */].Any, (_, inserted) => {
  ...
  */
  ...
  async payloadByDecryptingResponse(response, passwordRequestHandler, key) {
    ...
    for (const passwordCandidate of this.previousPasswords) {
      ...
      const key = await this.protocolService.computeRootKey(passwordCandidate, response.auth_params);

      if (!key) {
        continue;
      }

      const nestedResponse = await this.payloadByDecryptingResponse(response, passwordRequestHandler, key);
...
class protocol_service_SNProtocolService extends pure_service["a" /* PureService */] {
  ...
  */
  ...
  async payloadByDecryptingPayload(payload, key) {
    ...
    const operator = this.operatorForVersion(version);
    const encryptionParameters = Object(generator["c" /* CreateEncryptionParameters */])(payload);

    try {
      const decryptedParameters = await operator.generateDecryptedParameters(encryptionParameters, key);
      return Object(generator["e" /* CreateMaxPayloadFromAnyObject */])(payload, decryptedParameters);
    } catch (e) {
...
// CONCATENATED MODULE: ./lib/services/history/item_history_entry.ts
...
// CONCATENATED MODULE: ./lib/services/history/note_history_entry.ts
...
// CONCATENATED MODULE: ./lib/services/history/item_history.ts
...
class item_history_ItemHistory {
  constructor(entries) {
    this.entries = [];
  ...
  static FromJson(entryJson) {
    ...
    return new item_history_ItemHistory(entries);
  }
...
class history_session_HistorySession {
  ...
  static FromJson(historySessionJson) {
    if (historySessionJson) {
      ...
      uuids.forEach(itemUUID => {
        ...
        content.itemUUIDToItemHistoryMapping[itemUUID] = item_history_ItemHistory.FromJson(rawItemHistory);
      });
      return new history_session_HistorySession(content);
    } else {
      return new history_session_HistorySession();
    }
  }
  ...
  historyForItem(uuid) {
    ...
    if (!history) {
      history = new item_history_ItemHistory();
...
class history_manager_SNHistoryManager extends pure_service["a" /* PureService */] {
  constructor(itemManager, storageService, contentTypes, timeout) {
    super();
    this.contentTypes = [];
  ...
  async initializeFromDisk() {
    ...
    this.historySession = await this.storageService.getValue(StorageKey.SessionHistoryRevisions).then(historyValue => {
      return history_session_HistorySession.FromJson(historyValue);
    });
  ...
  async saveToDisk() {
    ...
    this.storageService.setValue(StorageKey.SessionHistoryRevisions, this.historySession);
  }
...
class application_SNApplication {
  ...
  */
  ...
  createHistoryManager() {
    this.historyManager = new history_manager_SNHistoryManager(this.itemManager, this.storageService, [content_types["a" /* ContentType */].Note], this.deviceInterface.timeout);
